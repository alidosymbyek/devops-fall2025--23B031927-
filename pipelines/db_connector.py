...
class DatabaseConnector:
    def __init__(self):
        # Read environment variables - detect if running locally or in Docker
        default_host = 'postgres'  # Docker network hostname
        default_port = '5432'       # Docker internal port
        
        # If running locally (not in Docker), use localhost and exposed port
        if not os.path.exists('/.dockerenv'):
            default_host = 'localhost'
            default_port = '5434'  # Docker exposes PostgreSQL on 5434
        
        self.host = os.getenv('DB_HOST', default_host)
        self.port = os.getenv('DB_PORT', default_port)
