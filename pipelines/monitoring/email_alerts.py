...
class EmailAlertManager:
    ...
    def send_pipeline_delay_alert(self, pipeline_name, expected_duration, actual_duration, threshold_percent=150):
        ...
        subject = f"⏱️ Pipeline Delay: {pipeline_name}"
        body = f"""
        <html>
        <body style="font-family: Arial, sans-serif;">
            <h2 style="color: #ff9800;">⏱️ Pipeline Delay Alert</h2>
            <table style="border-collapse: collapse; width: 100%;">
                <tr>
                    <td style="padding: 8px; background-color: #f5f5f5;"><strong>Pipeline:</strong></td>
                    <td style="padding: 8px;">{pipeline_name}</td>
                ...
                <tr>
                    <td style="padding: 8px; background-color: #f5f5f5;"><strong>Status:</strong></td>
                    <td style="padding: 8px; color: #ff9800;"><strong>DELAYED</strong></td>
                </tr>
                <tr>
                    <td style="padding: 8px; background-color: #f5f5f5;"><strong>Expected Duration:</strong></td>
                    <td style="padding: 8px;">{expected_duration:.2f} seconds</td>
                </tr>
                <tr>
                    <td style="padding: 8px; background-color: #f5f5f5;"><strong>Actual Duration:</strong></td>
                    <td style="padding: 8px;">{actual_duration:.2f} seconds</td>
                </tr>
                <tr>
                    <td style="padding: 8px; background-color: #f5f5f5;"><strong>Delay:</strong></td>
                    <td style="padding: 8px; color: #ff9800;">{delay_percent:.1f}% over expected time</td>
                </tr>
                <tr>
                    <td style="padding: 8px; background-color: #f5f5f5;"><strong>Time:</strong></td>
                    <td style="padding: 8px;">{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</td>
            ...
            <p style="margin-top: 20px;">
                <strong>Action Required:</strong> Pipeline is taking longer than expected. Please investigate.
            </p>
            <p>
                <a href="http://localhost:8080" style="background-color: #1976d2; color: white; padding: 10px 20px; text-decoration: none; border-radius: 4px;">
                    View in Airflow
