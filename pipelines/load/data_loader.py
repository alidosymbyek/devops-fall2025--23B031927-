...
class DataLoader:
    def __init__(self, write_files=False):
        ...
        # Setup processed directory if file writing is enabled
        if self.write_files:
            self.processed_dir = Path(__file__).parent.parent.parent / 'data' / 'processed'
            self.processed_dir.mkdir(parents=True, exist_ok=True)
            logger.info(f"Processed file writing enabled: {self.processed_dir}")
    ...
    def load_to_staging(self, df, table_name, schema='staging'):
        ...
        try:
            # Optionally write processed file before database load
            if self.write_files and schema == 'staging':
                self._write_processed_file(df, table_name)
    ...
    def _write_processed_file(self, df, table_name):
        """Write processed data to processed directory as CSV"""
        try:
            ...
            logger.info(f"✓ Written processed file: {filepath}")
        except Exception as e:
            logger.warning(f"Failed to write processed file: {e}")
